...
int main(int argc, char **argv) {
  ...
  while (!fin.eof()) {
    ...
    Eigen::Isometry3d Twr(Eigen::Quaterniond(qw, qx, qy, qz)); //SE3 Matrix for Rotation
    Twr.pretranslate(Eigen::Vector3d(tx, ty, tz)); // A.translate(B) => A*B A.pretranslate(B) => B*A  ==== SE3 Matrix Translation&Rotation
    poses.push_back(Twr);
  }
